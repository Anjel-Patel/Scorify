...
function Rowmaker(details) {
  ...
  return details.map((info, i) => (
    <PersonRow
      ...
      name={info.fullname}
      role={info.role}
      normalHours={info.totaltime}
      overtimeHours={info.totalovertime}
      score={info.totalscore}
    />
  ));
}

function Rankingcard(prop) {
  const d = prop.leaderBoardInfo.map((info, i) => {
    if (info.fullname === prop.name) info.isSelf = true;
    else info.isSelf = false;
    return info;
  });
  // const details = [
  ...
  return (
    <div className="rank-card">
      <div className="title-wrapper">
        ...
        <h5 className="h5 title" style={{ marginLeft: "8px" }}>
          {prop.department}
        </h5>
      </div>
      ...
      <div className="header" style={{ width: "1440px", marginLeft: "0px" }}>
        <h5 className="h5 head" style={{ left: "56px" }}>
          Rank
        </h5>
        <h5 className="h5 head" style={{ left: "154px" }}>
          Name
        </h5>
        <h5 className="h5 head" style={{ left: "922px" }}>
          Normal hours
        </h5>
        <h5 className="h5 head" style={{ left: "1088px" }}>
          Overtime hours
        </h5>
        <h5 className="h5 head" style={{ left: "1270px" }}>
          Role
        </h5>
        <h5 className="h5 head" style={{ left: "1395px" }}>
          Score
        </h5>
      </div>

      <div className="rows">{Rowmaker(d)}</div>
    </div>
  );
}
