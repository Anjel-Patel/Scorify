...
function PhoneNumberGenerator(phoneNumbers, editMode, logger){
    ...
    return phoneNumbers.map((phno, i) => (
        ...
        <div style={{display:'flex', alignItems:'center'}}>
            <input key ={i} className="phno h3 field" name = {pname[i]} value={phno} onChange={logger} readOnly={editMode===0?true:false}></input>
        ...
        <div className="personal-page-rect" style={editMode===0?{cursor:'default', caretColor:'transparent'}:{}}>
            ...
            <div className="profile-card">
                ...
                <div className="left-side" style={editMode===1?editableStyle:{}}>                    
                    <input className="name h1 field-name" name = 'fullName' value={info.fullName} onChange={logger} readOnly={editMode===0?true:false}></input>
                    <div className="detail-grid">
                        <div className="id detail-div">
                            <p className="p1 detail-title">ID</p>
                            <input className="h3 field" value={info.empID} readOnly={true}></input>
                        </div>
                        <div>
                            <p className="p1 detail-title">Email</p>
                            <input className="email h3 field" name= 'emailID' value={info.emailID} onChange={logger} readOnly={editMode===0?true:false}></input>
                        </div>
                        <div>
                            <p className="p1 detail-title">DOB</p>
                            <input className="dob h3 field" name = 'DateOfBirth' value={info.DateOfBirth} onChange={logger} readOnly={editMode===0?true:false}></input>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input className="gender h3 field" name ='Sex' value={info.Sex} onChange={logger} readOnly={editMode===0?true:false}></input>
                        </div>
                    </div>
                    <div className="address" >
                            <p className="p1 detail-title">Address</p>
                            <textarea rows={4} className="address p1 address-text field" name= 'address' value={info.address} onChange={logger} readOnly={editMode===0?true:false}></textarea>
                    </div>
                </div>
                ...
                <div className="right-side" style={{cursor:'default'}}>
                    ...
                    <div className="stats-bottom-wrapper">
                        ...
                        {/* Save button */}
                        <div className="save-btn" style={editMode===0?{display:'none', cursor:'default'}:{}} onClick={updateInfo}>
