...
function RecordRow({
  ...
  return (
    ...
    >
      <h5 className="p1 leader-board-details" style={{ left: "24px" }}>
        {name}
      </h5>
      <h5 className="p1 leader-board-details" style={{ left: "201px" }}>
        {role}
      </h5>
      <h5
        className="p1 leader-board-details"
        style={{ left: "495px", color: "var(--neutral-500)" }}
      >
        {prev_normalHours}
      </h5>
      <h5
        className="p1 leader-board-details"
        style={{ left: "654px", color: "var(--neutral-500)" }}
      >
        {prev_overtimeHours}
      </h5>
      <h5
        className="p1 leader-board-details"
        style={{ left: "777px", color: "var(--neutral-500)" }}
      >
        {prev_score}
      </h5>
      <input
        className="p1 leader-board-details record-field"
        name="overtimeHours"
        style={isValid1 === 1 ? errorColor : { left: "973px" }}
        value={overtimeHours}
        onChange={(e) => {
          overtimeLogger(e, details, date, empID, setDetails);
      ...
      <div className="record-attendance-rect" style={{ left: "1088px" }}>
        <div
          id="present"
          className={
            "record-attendance-present-rect" +
            (attendance === 1 ? " present-selected" : "")
          }
          name="present"
          onClick={(e) => {
            attendanceLogger(
        ...
        <div
          id="absent"
          className={
            "record-attendance-absent-rect" +
            (attendance === 0 ? " absent-selected" : "")
          }
          name="absent"
          onClick={(e) => {
            attendanceLogger(
