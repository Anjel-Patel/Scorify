...
const [
  ...
  getFullName,
  getDepartment,
  getCurrentRecordMembers,
  getCurrentRecordLeaders,
  getDateLeader,
  getLUWeekNoLeader,
  getLUWeekNoManager,
  getDateManager,
...
app.post("/updatedrecords", (req, res) => {
  ...
  for (empID of empIDlist) {
    ...
    if (records[empID]["attendance"] === 0) {
      ...
      if (
        records[empID]["overtimeHours"] !== "0" &&
        records[empID]["overtimeHours"] !== "-"
      ) {
...
app.put("/updatedinfo", (req, res) => {
  const {
    infohalf: { fullName, empID, emailID, DateOfBirth, Sex, address },
    phno,
  } = req.body;
